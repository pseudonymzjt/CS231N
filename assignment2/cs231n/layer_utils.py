...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_params):
    """
    Convenience layer that perorms an affine transform, normalizes with batchnorm followed by a ReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta, bn_params: params for the batchnorm normalization
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    da, dgamma, dbeta = batchnorm_backward_alt(dbn_out, bn_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db, dgamma, dbeta

def affine_layernorm_relu_forward(x, w, b, gamma, beta, ln_params):
    """
    Convenience layer that perorms an affine transform, normalizes with layernorm followed by a ReLU
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    ln_out, ln_cache = layernorm_forward(a, gamma, beta, ln_params)
